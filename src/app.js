...
function enter(e) { if (e.key === "Enter") navigate(address.value); }
function newTab() { createTab("portal.html"); }
function home() { tabs[current].iframe.src = "portal.html"; }
function back() { tabs[current].iframe.contentWindow.history.back(); }
function forward() { tabs[current].iframe.contentWindow.history.forward(); }
function reload() { tabs[current].iframe.contentWindow.location.reload(); }
function changeYear(y) { WAYBACK_YEAR = y; navigate(address.value); }
